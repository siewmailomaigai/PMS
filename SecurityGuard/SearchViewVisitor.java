 ...
 * @author Hong Shen
 */
public class SearchViewVisitor extends VisitorRT{
    ArrayList<SearchViewVisitor> visitor = new ArrayList<>();
    Scanner input = new Scanner(System.in);
    public SearchViewVisitor() {
    }

    public SearchViewVisitor(String vID, String rtID, String name, String entryStatus, LocalDate date) {
        ...
        try {
            try (Scanner scan = new Scanner(new File(VISITORFILE))) {
                ...
                while (scan.hasNextLine()) {
                    ...
                    SearchViewVisitor vrt = new SearchViewVisitor();
                    if (parts.length >= 4) {
                        vrt.setvID(parts[0]);
                        vrt.setRtID(parts[1]);
                        vrt.setName(parts[2]);
